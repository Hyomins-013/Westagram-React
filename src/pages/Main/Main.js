...
const Main = () => {
  ...
  const [likeChange, setLikeChange] = useState('images/heart.png');
  // const [likeState, setLikeState] = useState(true);
  const [likeArray, setLikeArray] = useState([likeChange, likeChange])

  // function heartChange(a) {

  //   a = true ? (setLikeChange('/images/fillheart.png'), setLikeState(false)) : 
  //   (setLikeChange('/images/heart.png'), setLikeState(true))
  // }
          ...
          <article className="feeds">
                      ...
                      commentArray.map(function(a, i) {
                        return(
                          <li className="comment" key={ i }>
                            ...
                            <img className="smallHeart" src={ likeArray[i] } onClick={()=>{
                              let copy = [...likeArray];
                              copy[i] = 'images/heart.png' ? setLikeChange('images/fillheart.png') : setLikeChange('images/heart.png');
                              setLikeArray(copy)
                              // heartChange(likeArray[i]);
                            }}/>
                            <span className="miniSubGray" onClick={()=>{
                              let copyComment = [...commentArray];
                              let copyLike = [...likeArray];
                              copyComment.splice(i, 1);
                              copyLike.splice(i, 1);
                              setCommentArray(copyComment);
                              setLikeArray(copyLike);
                            }}>댓글삭제</span>
                          </li>
                        )
                        })
            ...
            <div className="commentDiv">
              <input id="commentInput"
                ...
                placeholder="댓글달기..."
              />
              <button id="commentReply" type="submit" onClick={() => { 
                let copyComment = [...commentArray];
                let copyLike = [...likeArray];
                copyComment.unshift(inputComment);
                copyLike.unshift(likeChange);
                setCommentArray(copyComment);
                setLikeArray(copyLike);
                setInputComment("");
               } }>
